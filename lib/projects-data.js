...
export const projectsDetail = [
  {
    ...
    liveDemo: null,
    techStack: ["Python", "FastAPI", "Docker SDK", "LangGraph", "LangChain v1", "SQLite", "Pydantic v2", "Typer CLI", "MCP stdio", "Next.js"],
    badge: "SRE OPERATOR",
    primaryImage: "/projects/arbiter/preview.png",
    stats: [
      ...
      { label: "AGENT CONTROLS", value: "TYPED OPERATIONS / COMPENSATING TRANSACTIONS" },
    ],
    summary: "Arbiter is a local-first Linux operations control plane engineered to understand, correlate, and support safety-oriented operation of complex developer workstations. It addresses port collisions, container sprawl, and service dependencies across Docker Compose stacks, standalone containers, and native host processes through deterministic allocation, persisted approvals, and rollback-aware configuration editing.",
    overview: [
      ...
      "Arbiter uses safety-oriented controls including typed operations instead of arbitrary shell execution, scoped filesystem writes, UUID-tracked approvals in SQLite, and compensating rollback transactions after failed Compose validation or container recreation. It remains under active development: these safeguards are designed to reduce the risk of unsafe or unintended system changes, but security cannot currently be guaranteed. Additional hardening, validation, observability, and application-specific safety checks are planned."
    ],
    problemStatement: {
      ...
      solution: "Arbiter provides a unified control plane that scans Linux `/proc` and socket listeners, maps Docker labels to Compose services, deterministically calculates free replacement ports, and rewrites Compose and `.env` files with validation and atomic backups.",
      keyTakeaway: "By separating observation, proposal, human approval, execution, and verification into distinct phases, Arbiter gives coding agents and developers a safety-oriented way to manage development environments while substantially reducing the risk of unsafe or unintended workstation changes."
    },
    architecture: {
...
+-------------------------------------------------------------------------+`,
    ...
    keyCapabilities: [
      ...
      },
      {
        ...
        description: "Edits Compose YAML and .env files with pre-flight syntax validation, timestamped backups, and automatic compensating rollbacks upon failure."
      },
      {
        title: "5-Tier Safety & Approval Engine",
        description: "Requires explicit cryptographic approval for state-changing operations and uses typed actions designed to prevent generic shell execution or active-volume deletion."
      },
      {
    ...
    screenshots: [
      {
        ...
        alt: "Arbiter Control Plane Dashboard Preview",
        caption: "Arbiter real-time control panel: listening port owners, container health metrics, and registered project status."
      }
    ],
    cliOrApiReference: {
    ...
    gettingStarted: {
      prerequisites: ["Python", "Docker & Docker Compose", "Linux / macOS"],
      steps: [
  ...
  },
  {
    ...
    liveDemo: null,
    techStack: ["Python", "FastAPI", "PostgreSQL", "Next.js", "Alembic", "GH Archive", "GitHub REST API", "Docker Compose", "Ruff", "MyPy"],
    badge: "BIG DATA PIPELINE",
    primaryImage: "/projects/repotrajectory/architecture.png",
    stats: [
    ...
    summary: "RepoTrajectory is an explainable research platform that tracks momentum and governance-health signals for open-source software. By combining high-level GitHub REST data with durable GH Archive hourly ingestion, RepoTrajectory separates automation noise from human contributions and computes transparent, reproducible metric snapshots.",
    overview: [
      ...
      "Every calculated score (Momentum, Community Health, Contributor Concentration Risk) is constrained to a 0–100 normalized range and persisted with its formula breakdown, sample sizes, and calculation timestamp so the scoring remains inspectable."
    ],
    problemStatement: {
...
+-------------------------------------------------------------------------+
                                     ...
                                     ▼
+-------------------------------------------------------------------------+
| DURABLE COLLECTOR ENGINE (Python CLI / Worker)                     |
...
+-------------------------------------------------------------------------+`,
      layers: [
        { name: "Asymmetric Collector Queue", description: "PostgreSQL job ledger with priority-based worker leasing, rate limit reserving, and idempotent daily snapshot deduplication.", tech: "PostgreSQL, SQLAlchemy, Python" },
    ...
    keyCapabilities: [
      ...
      {
        title: "Recoverable Durable Ledger",
        description: "Uses conflict-safe PostgreSQL upserts and replaceable hourly projections so collection can resume after crashes or worker restarts without relying on a separate message broker."
      }
    ],
    technicalDeepDive: [
      {
      ...
      },
      {
        ...
        content: "Rather than introducing Redis or Celery, RepoTrajectory uses PostgreSQL row-level locks (SELECT ... FOR UPDATE SKIP LOCKED) to coordinate distributed collector workers. The queue applies explicit priority ordering (Pinned > Discovery > Archive > Hydration) without requiring separate external messaging infrastructure."
      }
    ],
    screenshots: [
      {
        src: "/projects/repotrajectory/architecture.png",
        alt: "RepoTrajectory System Architecture Diagram",
        caption: "RepoTrajectory full software architecture: GH Archive hourly ingestion, PostgreSQL job queue with worker leases, and explainable scoring pipeline."
      },
      {
        src: "/projects/repotrajectory/preview.png",
        alt: "RepoTrajectory Analytics Dashboard Preview",
        caption: "RepoTrajectory analytics dashboard displaying repository momentum, contributor concentration, and community health."
      }
    ],
    cliOrApiReference: {
    ...
    gettingStarted: {
      prerequisites: ["Python", "PostgreSQL 16+", "Node.js 20+", "Docker Compose"],
      steps: [
  ...
  },
  {
    ...
    liveDemo: null,
    techStack: ["Python", "FastAPI", "Next.js", "PostgreSQL", "LangGraph", "LangChain", "OpenAI GPT-5", "Playwright", "Alembic", "Docker"],
    badge: "RELIABILITY CONSOLE",
    primaryImage: "/projects/gitaudit/preview.png",
    stats: [
      { label: "CI CORRELATION", value: "EXACT DEFAULT-BRANCH COMMIT SHA" },
      { label: "SAFETY BOUNDARY", value: "HUMAN-REVIEWED PROPOSALS ONLY" },
...
+-------------------------------------------------------------------------+`,
      layers: [
        ...
        { name: "Scan & Health Normalizer", description: "Evaluates exact-commit checks, separates signal quality from coverage, and handles GitHub API degradation gracefully.", tech: "Python, FastAPI, GitHub REST / GraphQL" },
    ...
    keyCapabilities: [
      ...
      },
      {
        ...
        description: "Keeps AI-generated changes proposal-only: suggestions are stored in PostgreSQL for developer inspection and are not automatically written to GitHub."
      },
      {
    ...
    screenshots: [
      {
        src: "/projects/gitaudit/preview.png",
        alt: "GitAudit Operations Console Preview",
        caption: "GitAudit reliability console displaying commit-exact default-branch CI verification, scan history, and health signals."
      }
    ],
    cliOrApiReference: {
    ...
    gettingStarted: {
      prerequisites: ["Python", "Node.js 20+", "PostgreSQL 16+", "Docker"],
      steps: [
  ...
  },
  {
    ...
    primaryImage: "/projects/forma/00-forma-architecture.png",
    stats: [
      { label: "PRIVACY MODEL", value: "LOCAL-FIRST STORAGE (DATA EXCLUDED FROM GIT)" },
    ...
    overview: [
      ...
      "Every company insight is retained with direct web source links, confidence scores, and identified organizational risks. Forma stores private résumés, contact details, signatures, and SQLite databases under local-data/ on the user's host machine; calls to configured AI or search providers remain subject to those providers' data-handling practices."
    ],
    problemStatement: {
      problem: "Applying for technical roles requires tailored résumés and authentic, company-specific cover letters. Many cloud-based AI tools require users to share personal context and can generate generic drafts without clear source support.",
      solution: "Forma coordinates bounded LangGraph workflows from a local workspace, pairing web-grounded company research with human decision gates to produce source-backed documents.",
    ...
    architecture: {
      description: "Forma uses a FastAPI backend running LangGraph agent workflows alongside a responsive React frontend. Application data is stored in a local SQLite database, and the application does not add its own cloud telemetry.",
...
+-------------------------------------------------------------------------+`,
      layers: [
        ...
        { name: "Local-First Persistence", description: "SQLite storage with atomic backup commands, with project data excluded from Git and Docker build contexts.", tech: "SQLite 3, Pydantic" }
      ]
    },
    keyCapabilities: [
      {
      ...
      {
        title: "Local-First Storage Model",
        description: "Stores resumes, applications, documents, and API keys in local-data/ and excludes that directory from Git and Docker build contexts. AI-powered operations may send task-relevant content to configured model or search providers."
      },
      {
    ...
    technicalDeepDive: [
      ...
      },
      {
        ...
        content: "To reduce the risk of data loss without running a separate database server, Forma uses SQLite's online backup API (.backup). Users can create point-in-time snapshots while the application is running without locking database readers."
      }
    ],
    screenshots: [
      {
    ...
    gettingStarted: {
      prerequisites: ["Docker & Docker Compose", "Python", "Node.js 22+"],
      steps: [
  ...
  },
  {
    ...
    badge: "MCP SERVER",
    primaryImage: "/projects/gemini-mcp/preview.png",
    stats: [
...
+-------------------------------------------------------------------------+`,
    ...
    technicalDeepDive: [
      ...
      },
      {
        ...
        content: "To constrain file access to the designated repository root and reduce exposure of host credentials, file-reading and search tools pass through path canonicalization checks that reject paths resolving outside that root."
      }
    ],
    screenshots: [
      {
        src: "/projects/gemini-mcp/preview.png",
        alt: "Gemini-MCP Graph Visualization UI & Tools",
        caption: "Gemini-MCP visual graph explorer and repository intelligence control panel."
      }
    ],
    cliOrApiReference: {
    ...
    gettingStarted: {
      prerequisites: ["Python", "Google Gemini API Key"],
      steps: [
  ...
  },
  {
    ...
    primaryImage: "/projects/rsvp-shift/01_focus_mode.png",
    stats: [
      ...
      { label: "PRIVACY", value: "ON-DEVICE TEXT PROCESSING" }
    ],
    ...
    overview: [
      "Reading lengthy online articles, documentation, or technical papers requires continuous eye movement across words and line breaks, which can make sustained reading feel demanding.",
      "RSVP Shift reduces visual repositioning by presenting words one by one at a fixed screen coordinate and highlighting an Optimal Recognition Point (ORP) near the center of each word.",
      "Built on Chrome Manifest V3, RSVP Shift requires no external network permissions and processes highlighted text on the local device. It features punctuation-adaptive pacing, keyboard-first navigation, and a scrubbable timeline."
    ],
    problemStatement: {
      ...
      keyTakeaway: "RSVP and consistent focus-letter alignment can increase the displayed reading pace while giving users direct control over timing and navigation."
    },
    architecture: {
...
+------------------------------------+   +--------------------------------+`,
    ...
    keyCapabilities: [
      ...
      },
      {
        ...
        description: "Uses no external analytics or tracking and makes no extension-initiated remote calls; selected text is processed within the local extension context."
      }
    ],
    technicalDeepDive: [
      {
        title: "Calculating the Optimal Recognition Point",
        content: "Common RSVP implementations place the focal point slightly left of a word's center. RSVP Shift calculates an ORP index for each word and shifts the container horizontally so the highlighted focus letter remains at a consistent screen position."
      },
      {
        title: "Manifest V3 & Local Text Processing",
        content: "RSVP Shift is architected under Chrome Manifest V3 without unsafe-eval or external script loading. The extension requests minimal permissions (activeTab, storage), and its reading pipeline is designed to keep highlighted text processing on the local device."
      }
    ],
    screenshots: [
      {
      ...
      },
      {
        ...
        caption: "The RSVP Shift popup provides practical reading controls with a bold red visual identity."
      },
      {
      ...
      },
      {
        ...
        caption: "Keyboard-first controls stay available when needed and out of the way while reading."
      },
      {
        src: "/projects/rsvp-shift/preview.png",
        alt: "RSVP Shift Extension Preview",
        caption: "RSVP Shift extension visual preview."
      }
    ],
    cliOrApiReference: {
  ...
  },
  {
    id: "07",
    slug: "portfolio",
    aliases: ["hazemhassine-space", "hazemhassine.space", "website"],
    title: "HAZEMHASSINE.SPACE",
    subtitle: "Brutalist personal space & interactive lab",
    headline: "High-performance personal engineering portfolio featuring brutalist design, interactive 3D WebGL physics, a Three.js wireframe arcade game, Markdown CMS, and an AI assistant.",
    category: "CREATIVE ENGINEERING & WEB",
    status: "PRODUCTION / OPEN SOURCE",
    year: "2026",
    timeline: "2026",
    github: "https://github.com/HazemHassine/hazemhassine.space",
    liveDemo: "https://hazemhassine.space",
    techStack: ["Next.js", "React", "TailwindCSS", "Three.js", "Framer Motion", "WebGL", "FastAPI"],
    badge: "PORTFOLIO & LAB",
    primaryImage: "/projects/portfolio/preview.png",
    stats: [
      { label: "FRAMEWORK", value: "NEXT.JS APP ROUTER (TURBOPACK)" },
      { label: "3D & VISUALS", value: "THREE.JS + WEBGL VAPORWAVE GAME" },
      { label: "ANIMATIONS", value: "FRAMER MOTION + BRUTALIST GRID" },
      { label: "DESIGN LANGUAGE", value: "INDUSTRIAL CYBERPUNK / HIGH CONTRAST" },
    ],
    summary: "hazemhassine.space is a personal digital workshop and portfolio designed to reflect a passion for systems engineering, artificial intelligence, and creative frontend development. It blends high-performance server-side rendering with tactile micro-interactions, responsive 3D WebGL scenes, and an interactive 404 game.",
    overview: [
      "Rather than relying on generic corporate portfolio templates, hazemhassine.space was architected from the ground up to showcase technical depth, attention to detail, and a distinct brutalist aesthetic.",
      "The site incorporates an interactive 3D WebGL glowing polygon hero, a retro wireframe runner game embedded directly in the 404 experience, and deep technical showcase pages with live screenshot lightboxes and architecture topology maps.",
      "Designed for fast page loads and stable hydration, the application uses Next.js server components, selective static generation, and Tailwind CSS variables for high-contrast accessibility."
    ],
    problemStatement: {
      problem: "Standard developer portfolios often feel interchangeable and static, failing to convey both deep backend architecture proficiency and modern, creative frontend engineering skill.",
      solution: "hazemhassine.space combines structured systems engineering case studies with interactive 3D shaders, WebGL game engines, and full-stack performance optimizations.",
      keyTakeaway: "A personal website should be a living technical artifact that demonstrates architecture, design taste, and craftsmanship in practice."
    },
    architecture: {
      description: "The site is built with Next.js App Router and React 19, structured around clean modular components, Server-Side Static Generation (SSG), dynamic SEO metadata, and decoupled client-side 3D WebGL viewports.",
      image: null,
      asciiDiagram: `
+-------------------------------------------------------------------------+
| CLIENT BROWSER: Next.js App Router (Turbopack Engine)                   |
+-------------------------------------------------------------------------+
                                     │
                 ┌───────────────────┼───────────────────┐
                 ▼                   ▼                   ▼
+-----------------------+ +---------------------+ +-----------------------+
| BRUTALIST UI ENGINE   | | THREE.JS 3D CANVAS  | | CONTENT ENGINE        |
| - Tailwind Variables  | | - Hero 3D Polyhedron| | - Markdown Blog Parser|
| - Framer Motion       | | - 404 Glider Game   | | - Case Study Data     |
| - Custom Cursor & Grid| | - Web Audio Synth   | | - Sitemap & Metadata  |
+-----------------------+ +---------------------+ +-----------------------+
                                     │
                                     ▼
+-------------------------------------------------------------------------+
| ROUTING & INTEGRATION LAYER                                             |
| - Dynamic Project Showcase Routes (/projects/[slug])                    |
| - Top-Level Path Rewrites (/arbiter, /repotrajectory, /gitaudit, etc.)  |
| - Serverless API Endpoints (Chat Assistant, Contact Relay)              |
+-------------------------------------------------------------------------+`,
      layers: [
        { name: "Next.js App Router & SSG", description: "Static page pre-rendering, route handlers, and dynamic metadata generation for lightning-fast loads.", tech: "Next.js, React" },
        { name: "Three.js & WebGL Visuals", description: "Hardware-accelerated 3D meshes with interactive mouse tracking and a retro synthwave terrain runner.", tech: "Three.js, WebGL, Web Audio API" },
        { name: "Brutalist Design System", description: "High-contrast neon green on pitch dark theme with noise overlays, animated scanlines, and tactile typography.", tech: "TailwindCSS, CSS Custom Properties" },
        { name: "Markdown Content Engine", description: "Frontmatter parser and dynamic blog reader supporting localized assets and code highlighting.", tech: "gray-matter, Markdown" }
      ]
    },
    keyCapabilities: [
      {
        title: "Interactive 3D WebGL Canvas",
        description: "Features custom Three.js viewport interactions that react to keyboard typing and mouse movement in real time."
      },
      {
        title: "Hidden 404 Retro Cyber Glider Game",
        description: "Embeds a full Three.js endless runner on 404 routes complete with procedural terrain generation and retro Web Audio sound synthesizer."
      },
      {
        title: "Deep Technical Showcase Views",
        description: "Provides multi-tabbed architectural breakdowns, ASCII topologies, CLI references, and fullscreen screenshot lightbox modals."
      },
      {
        title: "Selective Static Generation",
        description: "Pre-renders eligible static paths at build time while server-rendering CMS-backed routes that require current content."
      },
      {
        title: "Integrated AI Chat Assistant",
        description: "Includes an embedded conversational agent capable of answering questions about portfolio projects, experience, and background."
      },
      {
        title: "Mobile-First Responsive Layout",
        description: "Optimized touch navigation, swipe-friendly menus, and refactored mobile headers designed for high readability on any screen."
      }
    ],
    technicalDeepDive: [
      {
        title: "Interactive Web Audio Synthesizer in 404 Game",
        content: "The embedded 404 wireframe glider game avoids external audio asset dependencies by synthesizing all sound effects in real time using the Web Audio API (AudioContext). It programmatically generates laser sweeps, coin chimes, and collision noise filters with zero network egress."
      },
      {
        title: "Top-Level Route Rewriting & Static Optimization",
        content: "To give each project a clean top-level identity while preserving unified Next.js App Router semantics, next.config.mjs maps vanity routes (/arbiter, /repotrajectory, /gitaudit, /forma, /gemini-mcp, /rsvp-shift, /portfolio) directly to /projects/[slug] with static pre-rendering."
      }
    ],
    screenshots: [
      {
        src: "/projects/portfolio/preview.png",
        alt: "HAZEMHASSINE.SPACE Preview",
        caption: "hazemhassine.space portfolio dashboard: hero presentation, project case studies, and brutalist interface."
      }
    ],
    cliOrApiReference: {
      title: "Local Development Commands",
      items: [
        { command: "npm run dev", description: "Start the Next.js development server with Turbopack on http://localhost:3000." },
        { command: "npm run build", description: "Execute a production build and pre-render all static project and blog pages." },
        { command: "npm run lint", description: "Run ESLint to check for syntax and accessibility rule compliance." }
      ]
    },
    gettingStarted: {
      prerequisites: ["Node.js 20+", "npm / pnpm / yarn"],
      steps: [
        { title: "1. Clone Repository", code: "git clone https://github.com/HazemHassine/hazemhassine.space.git\ncd hazemhassine.space" },
        { title: "2. Install Dependencies", code: "npm install" },
        { title: "3. Start Local Server", code: "npm run dev" }
      ]
    }
  }
];
